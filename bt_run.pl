...
my $executable = './BayesTraitsV3.0.2-OSX/BayesTraitsV3';
my $script1 = './indep.txt';
my $script2 = './dep.txt';

my $file_trait = './trait.txt';
my $file_genes = './matrix_geneStatus.tsv';
my $file_tree = './tree.nex';
...
foreach my $line (<DATA>) {
	...
	foreach (@array) {
		next if $_ =~ /\D/;
		$n++ if $_ == 1; #ignoring missing identifiers (ie. NA, x, -)
	}	
	$n1{$string} = $n;
